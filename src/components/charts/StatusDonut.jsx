...
export default function StatusDonut({ data = [] }) {
  const totalLeads = data.reduce((acc, item) => acc + (item.value || 0), 0);

  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const item = payload[0];
      const pct = totalLeads > 0 ? ((item.value / totalLeads) * 100).toFixed(1) : 0;
      return (
  ...
  return (
    <div className="glass-card" style={{ padding: '24px' }}>
      <div style={{ marginBottom: '16px' }}>
        <h3 style={{ fontSize: '17px', color: 'var(--text-primary)' }}>Lead Pipeline Distribution</h3>
        <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '2px' }}>
          Current status of all leads in the CRM funnel
        </p>
      </div>
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: item.fill }}></span>
